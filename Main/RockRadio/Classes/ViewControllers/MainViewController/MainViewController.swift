...
class MainViewController: UIViewController, UNUserNotificationCenterDelegate {
    ...
    let stations = [Station(name: appName,
                            ...
                            image: UIImage(named: "hosanna logo"))]
    ...
    var track: Track? {
        didSet {
            currentSongLabel.text = track?.name?.replacingOccurrences(of: "%20", with: " ", options: .literal, range: nil)
//                + " - " + (track?.name ?? "")
            radioInformation.text = (track?.artist ?? "").replacingOccurrences(of: "%20", with: " ", options: .literal, range: nil)
            updateNowPlaying(with: track)
        }
    }
...
//        self.gifImageView.image = UIImage.gif(name: "jix6go84T")
...
//        self.apiCall()
    ...
    func getSpotifyData() {
        ...
        Firestore.firestore().collection("spotify").getDocuments { (documentSnapshot, error) in
            if let err = error{
                print("Error getting documents: \(err)")
            }else{
                for document in documentSnapshot!.documents {
                    let data = document.data()
                    self.spotifyObject.append(data)
                }
...
//                self.adsData()
    ...
    func adsData() {
        ...
        Firestore.firestore().collection("customAds").getDocuments { (documentSnapshot, error) in
            if let err = error{
                print("Error getting documents: \(err)")
            }else{
                for document in documentSnapshot!.documents {
    ...
    func updateAdsUI(ad: [String : Any]) {
        ...
        if let imageURl = ad["imageUrl"] as? String {
            ...
            self.adImageView.sd_setImage(with: URL(string: imageURl), placeholderImage: placeholderImage)
        }
...
//        playerImageView.backgroundColor = UIColor.darkBlack
        playerImageView.layer.cornerRadius = 10.0
...
//        albumCoverImage.contentMode = .scaleToFill
        albumCoverImage.layer.cornerRadius = 10.0
...
//            self.gifImageView.isHidden = false
    ...
    @IBAction func shareButton(_ sender: Any) {
        ...
        let text = "Hello! I listen to the \(appName) 📲 https://apps.apple.com/us/app/hellas-radio-uk/id1636734822\n\n"
    ...
    @IBAction func spotifyButtonTapped(_ sender: Any) {        
        let viewController = SpotifyViewController(nibName: "SpotifyViewController", bundle: nil)
        viewController.spotifyArray = self.spotifyObject
